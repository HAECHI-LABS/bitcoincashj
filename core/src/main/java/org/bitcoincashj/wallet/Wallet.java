...
import org.bitcoincashj.script.Script.ScriptType;
import org.bitcoincashj.signers.LocalSchnorrTransactionSigner;
import org.bitcoincashj.signers.MissingSigResolutionSigner;
import org.bitcoincashj.signers.TransactionSigner;
import org.bitcoincashj.utils.BaseTaggableObject;
import org.bitcoincashj.utils.ListenerRegistration;
import org.bitcoincashj.utils.Threading;
import org.bitcoincashj.wallet.Protos.Wallet.EncryptionType;
import org.bitcoincashj.wallet.WalletTransaction.Pool;
import org.bitcoincashj.wallet.listeners.*;
import org.bitcoincashj.wallet.selector.CoinSelector;
import org.bitcoincashj.wallet.selector.DefaultCoinSelector;
import org.bitcoincashj.wallet.selector.FilteringCoinSelector;
import org.bitcoincashj.wallet.selector.KeyTimeCoinSelector;
...
public class Wallet extends BaseTaggableObject
    ...
    /**
     * An alias for calling {@link #freshAddress(org.bitcoincashj.wallet.KeyChain.KeyPurpose)} with
     * {@link org.bitcoincashj.wallet.KeyChain.KeyPurpose#RECEIVE_FUNDS} as the parameter.
     */
     ...
     *
     * @throws org.bitcoincashj.core.ECKey.MissingPrivateKeyException if the seed is unavailable (watching wallet)
     */
    ...
    @GuardedBy("lock")
    private List<BalanceFutureRequest> balanceFutureRequests = new LinkedList<>();
    ...
    * If ensureMinRequiredFee is true, feePerKb is set to at least {@link Transaction#REFERENCE_DEFAULT_MIN_TX_FEE}.
    ...
    public List<TransactionOutput> calculateSpendCandidatesForAddress(Address address, boolean excludeImmatureCoinbases, boolean excludeUnsignable) {
        List<TransactionOutput> allUtxos = this.calculateAllSpendCandidates(excludeImmatureCoinbases, excludeUnsignable);
        List<TransactionOutput> candidates = new ArrayList<>();
        for(TransactionOutput output : allUtxos) {
            Address utxoAddress = output.getScriptPubKey().getToAddress(getParams());
            if(utxoAddress.toBase58().equals(address.toBase58())) {
                candidates.add(output);
            }
        }
     ...
     */
    public void setTransactionBroadcaster(@Nullable org.bitcoincashj.core.TransactionBroadcaster broadcaster) {
     ...
     * @throws org.bitcoincashj.wallet.DeterministicUpgradeRequiresPassword if key rotation requires the users password.
     */
